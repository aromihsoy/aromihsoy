<h1 align="center">
  Всем привет! Я Backend-разработчик
  <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="30" />
</h1>

<div align="center">
  
<a href="https://github.com/aromihsoy"><img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=20&pause=1000&color=2CA5E0&center=true&vCenter=true&width=435&lines=Backend+Developer;Python+%7C+FastAPI+%7C+PostgreSQL;IT+Infrastructure+Enthusiast&v=1" alt="Typing SVG" /></a>

</div>

Перехожу в backend-разработку из IT-поддержки и системного администрирования. Опыт работы с сетями, операционными системами и оборудованием даёт практическую базу: я понимаю, как приложение ведёт себя «под капотом» и как оно взаимодействует с реальной инфраструктурой, а не только с кодом.

Сейчас сосредоточен на чистой архитектуре, проектировании реляционных баз данных и построении стабильных серверных приложений.

### 🌱 Чем я занимаюсь

- Разрабатываю асинхронные веб-сервисы на **Python** (FastAPI).
- Проектирую схему БД и работаю с **PostgreSQL** через **SQLAlchemy**.
- Настраиваю кэширование на **Redis** и фоновые задачи через брокеры сообщений (**RabbitMQ**, Celery).
- Упаковываю приложения в **Docker** для воспроизводимого развёртывания.
- Тестирую и документирую API в **Postman**.

### 🚀 Что осваиваю

- Изучаю **Django** для разработки полноценных веб-приложений.
- Погружаюсь в **Go** для микросервисной разработки.

### 🛠 Мой стек технологий:

**IDE:**

![](https://skillicons.dev/icons?i=vscode,pycharm,visualstudio)

**Backend & Базы данных:**

![](https://skillicons.dev/icons?i=py,go,fastapi,django,githubactions,nginx,postgres,sqlite,redis,rabbitmq)

**Инфраструктура & Автоматизация:**

![](https://skillicons.dev/icons?i=docker,powershell,git)

**Тестирование & API:**

![](https://skillicons.dev/icons?i=postman)



### 📈 Статистика GitHub:
---

<div align="center">
  <img src="https://github-readme-stats-fast.vercel.app/api?username=aromihsoy&theme=tokyonight&show_icons=true&hide_border=true&count_private=true" alt="GitHub Stats" />
</div>

---

<div align="center">
  <a href="https://github.com/aromihsoy">
    <img src="https://streak-stats.demolab.com/?user=aromihsoy&theme=tokyonight&hide_border=true" alt="GitHub Streak" />
  </a>
  <a href="https://github.com/aromihsoy">
    <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=aromihsoy&theme=tokyonight" alt="Top Langs" />
  </a>
</div>

---

<div align="center">

![](https://github-profile-trophy-tawny.vercel.app/?username=aromihsoy&theme=radical&no-frame=true&no-bg=true&margin-w=4)

</div>

---

### 🐍 Моя активность:
<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/aromihsoy/aromihsoy/output/github-snake-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/aromihsoy/aromihsoy/output/github-snake.svg">
    <img alt="GitHub Snake" src="https://raw.githubusercontent.com/aromihsoy/aromihsoy/output/github-snake.svg">
  </picture>
</div>
