<h1 align="center">
  <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="30" />
</h1>

<div align="center">

<a href="https://github.com/aromihsoy"><img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=500&size=20&pause=1000&color=2CA5E0&center=true&vCenter=true&width=435&lines=Backend+Developer;Python+%7C+FastAPI+%7C+PostgreSQL;IT+Infrastructure+Enthusiast&v=1" alt="Typing SVG" /></a>

</div>

Backend-разработчик с опытом в IT-инфраструктуре - системном администрировании, сетях и поддержке.

Сейчас сосредоточен на чистой архитектуре, проектировании реляционных баз данных и построении стабильных серверных приложений.

В данный момент являюсь Front+Backend-разработчиком в стартапе.

### 🌱 Чем я занимаюсь

- Разрабатываю асинхронные веб-сервисы на **Python** (FastAPI).
- Проектирую схему БД и работаю с **PostgreSQL** через **SQLAlchemy**.
- Строю event-driven системы на **RabbitMQ**: ручная AMQP-топология,
  transactional outbox, идемпотентные консьюмеры, DLQ и retry с backoff.
- Настраиваю кэширование и Pub/Sub на **Redis**, фоновые задачи через **Celery**.
- Упаковываю приложения в **Docker** для воспроизводимого развёртывания.
- Тестирую и документирую API в **Postman**.

### 🚀 Что осваиваю

- Изучаю **Django** для разработки полноценных веб-приложений.
- Следующим планирую осваивать **Go** для микросервисной разработки.

### 🛠 Мой стек технологий:

**IDE:**

![](https://skillicons.dev/icons?i=vscode,pycharm)

**Backend & Базы данных:**

![](https://skillicons.dev/icons?i=py,fastapi,django,githubactions,nginx,postgres,sqlite,redis,rabbitmq)

**Инфраструктура & Автоматизация:**

![](https://skillicons.dev/icons?i=docker,powershell,git)

**Тестирование & API:**

![](https://skillicons.dev/icons?i=postman)



### 📈 Статистика GitHub:
---

<div align="center">
  <img src="https://raw.githubusercontent.com/aromihsoy/github-stats/generated/overview.svg" alt="Stats" />
  <img src="https://raw.githubusercontent.com/aromihsoy/github-stats/generated/languages.svg" alt="Languages" />
</div>

---

<div align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="https://raw.githubusercontent.com/aromihsoy/aromihsoy/output/github-snake-dark.svg">
    <source media="(prefers-color-scheme: light)" srcset="https://raw.githubusercontent.com/aromihsoy/aromihsoy/output/github-snake.svg">
    <img alt="GitHub Snake" src="https://raw.githubusercontent.com/aromihsoy/aromihsoy/output/github-snake.svg">
  </picture>
</div>
